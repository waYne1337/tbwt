...
class wt_rlmn
{
	...
	template<class t_rac>
        wt_rlmn(t_rac& text_buf, size_type size):m_size(size) {
            {
		...
		size_type n_runs = 0; //count number of runs
		...
		t_rac condensed_text_buf; condensed_text_buf.resize( n_runs );
                for (size_type i=0, j=0; i < size; ++i) {
                    value_type c = text_buf[i];
                    if (bl[i]) {
                        bf[lf_map[c]] = 1;
			condensed_text_buf[j++] = c;
                    }
                    ++lf_map[c];
                }
                {
                    m_wt = wt_type(condensed_text_buf, n_runs);
                }
